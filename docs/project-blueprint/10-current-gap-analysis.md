...
| A2 | Implemented (`application/a2_handlers.py`, `a2_worker_capabilities.py`) | Real command execution (git/pytest/ruff/pytest-benchmark, a requester-declared command, or a Compose-based evaluation protocol) via `LocalWorkerBroker`; command detection prioritizes the requester's own declared command, falls back to pyproject.toml convention, then mandatory human approval for LLM-suggested commands |
